        ...
        public ActionResult Upload(HttpPostedFileBase[] files)
        {
            ...
            if (files[0] != null)
            {
                ...
                if (files.Length <= 10)
                {
                    ...
                    //if they are all valid then save to disk
                    if (allValid)
                    {
                        foreach (var file in files)
                        {
                            ...
                            {
                                ModelState.AddModelError("FileName", "Sorry an error occured while saving the files to disk, " +
                                    "please try again.");
                            }
                        }
                    }
                    else //add an error listing the invalid files
                    {
                        ModelState.AddModelError("FileName", "All files must be gif, png, jpeg, or jpg and less than 2MB in size. The following files " +
            ...
            if (ModelState.IsValid)
            {
                ...
                //dd a list of duplicatefiles to the error message
                if (duplicates)
                {
                    ModelState.AddModelError("FileName", "All files uploaded except the files" + duplicateFiles + ", " +
                        "which already exist in the system." + "Please delete then and try again if you wish to re-add them.");
                    return View();
                }
                else if (otherDbError)
                {
                    ModelState.AddModelError("FileName", "Sorry an error occured while saving the files to the database, " +
                                    "please try again.");
                    return View();
                }
