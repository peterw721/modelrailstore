...
using _92031083_ModelRailStore.ViewModels;
...
namespace _92031083_ModelRailStore.Controllers
{
    ...
    public class ProductsController : Controller
    {
        ...
        //public ActionResult Create([Bind(Include = "ID,Name,Year,Description,Price,CategoryID")] Product product)
        ...
        {
            var productToUpdate = db.Products.Include(p => p.ProductImageMappings).Where(
                p => p.ID == viewModel.ID).Single();

            if (TryUpdateModel(productToUpdate, "", new string[] { "Name",
                "Description", "Price", "Year", "CategoryID" }))
            {
                if (productToUpdate.ProductImageMappings == null)
                {
                    productToUpdate.ProductImageMappings = new List<ProductImageMapping>();
                }
                //get a llst of selected Images without any Blanks
                string[] productImages = viewModel.ProductImages.Where(pi => !string.IsNullOrEmpty(pi)).ToArray();
                ...
                for (int i = 0; i < productImages.Length; i++)
                {
                    ...
                    var imageMappingToEdit =
                        productToUpdate.ProductImageMappings.Where(pim => pim.ImageNumber ==
                        i).FirstOrDefault();
                    //find the new lmage
            ...
            else
            {
                ...
                //delete any imagemappings that the user did not include in their selections for the product

                for (int i = productImages.Length; i < Constants.NumberOfProductImages; i++)
                {
                    var imageMappingToEdit = productToUpdate.ProductImageMappings.Where(
                        pim => pim.ImageNumber == i).FirstOrDefault();
                    //if there is something stored in the mapping
                    if (imageMappingToEdit != null)
                    {
                        //delete the record from the mapping table directly
                        //just calling productToUpdate.productimageMappings.Remove(imaheMappingToEdit) results
                        //in a FK error
